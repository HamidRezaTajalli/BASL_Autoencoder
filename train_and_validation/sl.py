...
def sl_training_procedure(tp_name, dataset, arch_name, cut_layer, base_path, exp_num, batch_size, alpha_fixed,
    ...
    #     torch.rand(size=(dataloaders['validation'].batch_size,) + input_batch_shape).to(device)).size()[1:]
    # summary(model=aut_enc_model, input_size=input_batch_shape, batch_size=dataloaders['validation'].batch_size)
    ...
    my_models = {'client': client_models, 'server': server_model, 'server2': server_model2,
                 'autoencoder': aut_enc_model, 'malicious1': malicious_client_model1,
                 'malicious2': malicious_client_model2}
    ...
    server_optimizer2 = optim.Adam(params=server_model2.parameters(), weight_decay=1e-4)
    ...
    malicious_client1_optimizer = optim.Adam(params=malicious_client_model1.parameters(), weight_decay=1e-4)
    malicious_client2_optimizer = optim.Adam(params=malicious_client_model2.parameters(), weight_decay=1e-4)

    aut_optimizer = optimizer = optim.Adam(aut_enc_model.parameters(), lr=1e-3)
    optimizers = {'client': client_optimizers, 'server': server_optimizer, 'server2': server_optimizer2,
                  'autoencoder': aut_optimizer, 'malicious1': malicious_client1_optimizer,
    ...
    if arch_name.upper() == "RESNET9" or arch_name.upper() == "RESNET18":
        ...
        server2_lr_scheduler = optim.lr_scheduler.LambdaLR(optimizer=server_optimizer2,
                                                           lr_lambda=lambda item: models.lr_schedule_resnet(item))

        mal_client1_lr_scheduler = optim.lr_scheduler.LambdaLR(optimizer=malicious_client1_optimizer,
                                                               lr_lambda=lambda item: models.lr_schedule_resnet(
                                                                   item))
        mal_client2_lr_scheduler = optim.lr_scheduler.LambdaLR(optimizer=malicious_client2_optimizer,
                                                               lr_lambda=lambda item: models.lr_schedule_resnet(
                                                                   item))
    else:
    ...
    aut_lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(aut_optimizer,
                                                            mode='min',
                                                            factor=0.2,
                                                            patience=20,
                                                            min_lr=5e-5)

    lr_schedulers = {'client': client_lr_schedulers, 'server': server_lr_scheduler, 'server2': server2_lr_scheduler,
                     'malicious1': mal_client1_lr_scheduler, 'malicious2': mal_client2_lr_scheduler,
                     'autoencoder': aut_lr_scheduler}
